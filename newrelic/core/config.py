...
# TODO 动态添加属性，这种设计方式适合将配置信息(固定的配置)对象化，“即插即用”，方便简介，可以学习
# TODO 然后再提供函数，将其扁平化，转化成字典
_settings = Settings() # TODO _settings也是全局单例模式，这种实现单例模式的方法适合配置变量
...
def _environ_as_set(name, default=''):
    # TODO 解析环境/配置文件变量为set类型，格式为 空格
    value = os.environ.get(name, default)
    return set(value.split())


def _environ_as_mapping(name, default=''):
    # TODO 解析环境变量为字典类型，格式为 key1:value1;key2:value2，返回键值数组
    result = []
...
def _parse_ignore_status_codes(value, target):
    # TODO 解析忽视的status_code的规则
    # TODO value 是字符串 target是集合
    items = value.split()
    for item in items:
        try:
...
def _parse_attributes(s):
    # TODO 校验环境变量名称是否合法，限制为256个字节，超过就会忽略这个配置环境变量
    valid = []
    for item in s.split():
...
def flatten_settings(settings):
    ...
    """
    # TODO 将_settings由Setting对象转换成字典
...
def apply_config_setting(settings_object, name, value, nested=False):
    ...
    """
    # TODO 添加配置变量，settings_object 是Settings类对象
...
def fetch_config_setting(settings_object, name):
    ...
    """
    # TODO 获取配置信息  settings_object 是Settings类对象
