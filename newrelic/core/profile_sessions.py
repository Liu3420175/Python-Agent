...
# TODO 线程分析器是一种低影响的性能分析工具，可在生产中使用它来识别应用程序中的瓶颈。 它通过定期（100毫秒）
#  捕获指定时间段内每个线程的堆栈跟踪来工作。 在指定的持续时间结束时，将堆栈跟踪汇总以构建一棵树。
#  树中的调用计数对应于相同上下文下函数在堆栈跟踪中出现的次数。
#  尽管调用树无法捕获整个执行，但是足够大的样本可以很好地表示应用程序的行为。 这样可以洞悉大部分时间都花在了应用程序的“热”功能上。
#  在此范围内，将删除采样率低于0.05％的条目。
