...
try:
    ...
    import configparser as ConfigParser  # TODO ini配置文件解析器，参考文档https://docs.python.org/zh-cn/3/library/configparser.html
...
_settings = newrelic.api.settings.settings() # TODO Settings对象
...
_config_object = ConfigParser.RawConfigParser() # TODO 单例模式，解析配置文件
...
_cache_object = [] # TODO 单例模式，缓存解析到的配置信息
...
_COMPRESSED_CONTENT_ENCODING = { # 压缩方式
...
def _process_setting(section, option, getter, mapper):
    #TODO 解析配置信息
    #TODO mapper就是上面定义的一些带map的函数
    # TODO getter是Configparse的方法
    try:
...
_configuration_done = False # TODO 配置信息有没有解析完
...
def _load_configuration(config_file=None, environment=None,
    ...
    if not _config_object.read([config_file]): # TODO 说明可以读取多个配置文件
        raise newrelic.api.exceptions.ConfigurationError(
    ...
    _process_setting('newrelic', 'log_file', 'get', None) # TODO 可以更改init的section
    ...
    if environment: # TODO 根据环境覆盖某些配置的值 ??????
        _process_setting('newrelic:%s' % environment,
    ...
    initialize_logging(log_file, log_level) # TODO 初始化日志格式，也可以自定义

    # Now process the remainder of the global configuration
    # settings.

    _process_configuration('newrelic') # TODO 处理ini文件里newrelic section下的变量，可以自定义
    ...
    for function in _settings.transaction_tracer.function_trace: # TODO 代理器会捕获附加的时间信息，格式module:function or module:class.function
        try:
            ...
            hook = _function_trace_import_hook(object_path, name, group,
                    label, params, terminal, rollup) # TODO 添加函数追踪钩子
    ...
    def _instrument(target):
        ...
        try:
            ...
            _module_import_hook_results[(target.__name__, module,
                    function)] = ''
            # TODO  如果导入/调用钩子失败，将失败的原因保存起来，并抛出异常
        except Exception:
            _module_import_hook_results[(target.__name__, module,
...
def _process_module_definition(target, module, function='instrument'):
    ...
    try:
        ...
        _raise_configuration_error(section)
    # TODO 如果之前没注册，配置文件也没有配置，直接注册钩子，并加入注册表
    try:
...
def _process_trace_cache_import_hooks():
    ...
    if GREENLET_HOOK not in _module_import_hook_results:
        pass
    elif _module_import_hook_results[GREENLET_HOOK] is None: # TODO 说明项目里没用到greenlet包
...
def _process_module_builtin_defaults():
    # TODO 模块绑定与钩子绑定，只需要自定义钩子，我们就可以监控各种框架
