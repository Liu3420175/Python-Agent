...
def import_module(name):
    __import__(name)  # TODO 动态导入模块，并返回模块包路径
    return sys.modules[name] # TODO sys.modules是一个全局字典，该字典是python启动后就加载在内存中。每当程导入新的模块，
    #TODO sys.modules都将记录这些模块。字典sys.modules对于加载模块起到了缓冲的作用，这样可以知道项目中用了哪些模块。
